...
void AShooterGameMode::Sxw_DebugShowTest(FString TestStr)
{
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, 8.f, FColor::Red, TestStr);
	}
}
...
void AShooterGameMode::DefaultTimer()
{
	...
	if (GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, 8.f, FColor::Red,GetMatchState().ToString());
	}
	...
	if (GetWorld()->IsPlayInEditor())
	{
		...
		if (GetMatchState() == MatchState::WaitingToStart)
		{
			...
			if (GEngine)
			{
				GEngine->AddOnScreenDebugMessage(-1, 8.f, FColor::Red, FString("PlayinEditor"));
			}
			StartMatch();
	...
	//if (GEngine)
	//{
	//	GEngine->AddOnScreenDebugMessage(-1, 8.f, FColor::Red, FString("WorldState Game"));
	//}
